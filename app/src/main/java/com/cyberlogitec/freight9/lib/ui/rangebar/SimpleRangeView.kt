...
open class SimpleRangeView @JvmOverloads constructor(
    ...
    private lateinit var leftHandleImage: Bitmap
    private lateinit var rightHandleImage: Bitmap
    ...
    init {
        ...
        leftHandleImage = BitmapFactory.decodeResource(r, R.drawable.img_start_indicator)
        rightHandleImage = BitmapFactory.decodeResource(r, R.drawable.img_end_indicator)
    }
    ...
    protected fun drawLines(canvas: Canvas) {
        ...
        //여기서 라인을 그린다
        drawActiveLine(canvas, xActiveStart, 0f, xActiveEnd, activeLineThickness)
    ...
    protected fun drawActiveThumbs(canvas: Canvas) {
        if (isRange) {
            drawActiveThumb(canvas, start, currentLeftFocusRadiusPx,true)
        }
        drawActiveThumb(canvas, end, currentRightFocusRadiusPx, false)
    }
    ...
    protected fun drawActiveThumb(canvas: Canvas, i: Int, size: ValueWrapper<Float>, isLeft: Boolean) {
        val x = getPositionX(i)
        var w = rightHandleImage.getWidth()
        var h = rightHandleImage.getHeight()

        lateinit var dst :Rect
        when(isLeft) {
            true->{
                dst = Rect(x.toInt()-w+1, 0, x.toInt()+1, h)
                canvas.drawBitmap(leftHandleImage, null, dst, null)}
            false->{
                dst = Rect(x.toInt(), 0, x.toInt()+w, h)
                canvas.drawBitmap(rightHandleImage, null, dst, null)}
