package com.cyberlogitec.freight9.lib.ui.rangebar
...
class RangeBarWithChart @JvmOverloads constructor(
    ...
    var chartSelectedBackgroundDrawable: Drawable =
            ContextCompat.getDrawable(context, R.drawable.fade_chart_bfbfbf)!!
    ...
    var chartUnselectedBackgroundDrawable: Drawable =
            ContextCompat.getDrawable(context, R.drawable.fade_chart_2b2b2b)!!
     ...
     * */
    var selectedSeekColor: Int = ContextCompat.getColor(context, R.color.colorChartSelectedLine)
        set(value) {
            field = value
            applyRangeBarStyle()
        }
    ...
    private fun styleDataSet(dataSet: LineDataSet, isSelected: Boolean = false): LineDataSet {
        if (!isSelected) {
            dataSet.apply {
                ...
                fillDrawable = chartUnselectedBackgroundDrawable
            }
        } else {
            dataSet.apply {
                fillColor = chartSelectedBackgroundColor
                color = chartSelectedLineColor
                fillDrawable = chartSelectedBackgroundDrawable
            }
        }
        dataSet.apply {
    ...
    private fun parseAttr(attrs: AttributeSet) {
        val typedArray =
                context.obtainStyledAttributes(attrs, R.styleable.RangeBarWithChart)

        chartSelectedBackgroundColor = typedArray.getColor(
                R.styleable.RangeBarWithChart_barChartSelectedBackgroundColor,
                chartSelectedBackgroundColor
        )

        chartUnselectedBackgroundColor = typedArray.getColor(
                R.styleable.RangeBarWithChart_barChartUnselectedBackgroundColor,
                chartUnselectedBackgroundColor
        )

        chartSelectedLineColor = typedArray.getColor(
                R.styleable.RangeBarWithChart_barChartSelectedLineColor,
                chartSelectedLineColor
        )

        chartUnSelectedLineColor = typedArray.getColor(
                R.styleable.RangeBarWithChart_barChartUnSelectedLineColor,
                chartUnSelectedLineColor
        )

        selectedSeekColor = typedArray.getColor(
                R.styleable.RangeBarWithChart_barActiveLineColor,
                selectedSeekColor
        )

        unselectedSeekColor = typedArray.getColor(
                R.styleable.RangeBarWithChart_barLineColor,
                unselectedSeekColor
        )

        thumbColor = typedArray.getColor(
                R.styleable.RangeBarWithChart_barThumbColor,
                thumbColor
        )

        thumbActiveColor = typedArray.getColor(
                R.styleable.RangeBarWithChart_barActiveThumbColor,
                thumbActiveColor
        )

        tickRadius = typedArray.getDimension(
                R.styleable.RangeBarWithChart_barActiveTickRadius,
                tickRadius
        )
