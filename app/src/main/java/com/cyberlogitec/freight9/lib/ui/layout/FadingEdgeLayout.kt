package com.cyberlogitec.freight9.lib.ui.layout
...
class FadingEdgeLayout : FrameLayout {
    ...
    private fun init(attrs: AttributeSet?, defStyleAttr: Int) {
        val defaultSize = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, DEFAULT_GRADIENT_SIZE_DP.toFloat(),
                resources.displayMetrics).toInt()
        if (attrs != null) {
            val arr = context.obtainStyledAttributes(attrs, R.styleable.FadingEdgeLayout, defStyleAttr, 0)
    ...
    override fun dispatchDraw(canvas: Canvas) {
        ...
        if (visibility == View.GONE || newWidth == 0 || newHeight == 0 || !fadeAnyEdge) {
    ...
    private fun initTopGradient() {
        ...
        val size = Math.min(gradientSizeTop, actualHeight)
        val l = paddingLeft
        val t = paddingTop
        val r = width - paddingRight
        val b = t + size
        gradientRectTop!![l, t, r] = b
        val gradient = LinearGradient(l.toFloat(), t.toFloat(), l.toFloat(), b.toFloat(), FADE_COLORS, null, Shader.TileMode.CLAMP)
        gradientPaintTop!!.shader = gradient
    }

    private fun initLeftGradient() {
        val actualWidth = width - paddingLeft - paddingRight
        val size = Math.min(gradientSizeLeft, actualWidth)
        val l = paddingLeft
        val t = paddingTop
        val r = l + size
        val b = height - paddingBottom
        gradientRectLeft!![l, t, r] = b
        val gradient = LinearGradient(l.toFloat(), t.toFloat(), r.toFloat(), t.toFloat(), FADE_COLORS, null, Shader.TileMode.CLAMP)
        gradientPaintLeft!!.shader = gradient
    }

    private fun initBottomGradient() {
        val actualHeight = height - paddingTop - paddingBottom
        val size = Math.min(gradientSizeBottom, actualHeight)
        val l = paddingLeft
        val t = paddingTop + actualHeight - size
        val r = width - paddingRight
        val b = t + size
        gradientRectBottom!![l, t, r] = b
        val gradient = LinearGradient(l.toFloat(), t.toFloat(), l.toFloat(), b.toFloat(), FADE_COLORS_REVERSE, null, Shader.TileMode.CLAMP)
        gradientPaintBottom!!.shader = gradient
    }

    private fun initRightGradient() {
        val actualWidth = width - paddingLeft - paddingRight
        val size = Math.min(gradientSizeRight, actualWidth)
        val l = paddingLeft + actualWidth - size
        val t = paddingTop
        val r = l + size
        val b = height - paddingBottom
        gradientRectRight!![l, t, r] = b
        val gradient = LinearGradient(l.toFloat(), t.toFloat(), r.toFloat(), t.toFloat(), FADE_COLORS_REVERSE, null, Shader.TileMode.CLAMP)
