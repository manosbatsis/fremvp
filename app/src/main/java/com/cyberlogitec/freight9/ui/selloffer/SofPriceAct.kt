...
import kotlinx.android.synthetic.main.act_sof_price.*
import kotlinx.android.synthetic.main.appbar_sof_price.*
...
@RequiresActivityViewModel(value = SofPriceVm::class)
class SofPriceAct : BaseActivity<SofPriceVm>() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.act_sof_price)
    ...
    private fun initView() {
        ...
        // set custom toolbar
        defaultbarInit(appbar_sof_price, menuType = MenuType.DONE, title = getString(R.string.selloffer_wizard_price))
    ...
    private fun setRxOutputs() {
                ...
                .subscribe {
                    ...
                    it?.let { contract ->
                        if (adapter.datas.isNullOrEmpty()) {
                            ...
                            // TODO : cost Value
                            contract.masterContractLineItems?.map {contractLineItem ->
                                contractLineItem.costPrice = contract.inventory?.inventoryDetails?.first { contractInventoryDetails ->
                                    contractLineItem.baseYearWeek == contractInventoryDetails.baseYearWeek
                                }?.inOutPrice?.toFloat()!!
                            }
        ...
        viewModel.outPuts.onClickNext()
                .bindToLifecycle(this)
                .subscribe {
                    Timber.d("f9: onClickNext (SofConditionAct)")
                    startActivity(Intent(this, SofConditionAct::class.java).putExtra(Intents.MSTR_CTRK, it as Contract))
                }
    }

    private fun recyclerViewInit() {
        recycler_volume.apply {
            layoutManager = LinearLayoutManager(this@SofPriceAct)
            adapter = this@SofPriceAct.adapter
        }
    }

    private fun setListener() {
        appbar_sof_price.toolbar_left_btn.setSafeOnClickListener{
            Timber.d("f9: toolbar_left_btn clcick")
            onBackPressed()
        }

        appbar_sof_price.toolbar_done_btn.setSafeOnClickListener {
        ...
        when (batchInputMode) {
            ...
            BatchInputMode.ModeBatchInputMax -> {
                ...
                tv_batch_max.setTextColor(ContextCompat
                        .getColor(this@SofPriceAct, if (isChecked) R.color.greyish_brown else R.color.very_light_pink))
            }
        }
