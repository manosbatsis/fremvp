...
import kotlinx.android.synthetic.main.appbar_bof_volume.*
import kotlinx.android.synthetic.main.body_batch.*
import kotlinx.android.synthetic.main.body_bof_volume.*
import kotlinx.android.synthetic.main.body_pol_pod_card_simple.*
import kotlinx.android.synthetic.main.bottom_batch_input.*
import kotlinx.android.synthetic.main.bottom_bof_volume.*
import kotlinx.android.synthetic.main.item_buy_offer_volume.view.*
...
@RequiresActivityViewModel(value = BofVolumeVm::class)
class BofVolumeAct : BaseActivity<BofVolumeVm>() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.act_bof_volume)
    ...
    private fun initView() {
        ...
        // set custom toolbar
        defaultbarInit(appbar_bof_volume, menuType = MenuType.DONE, title = getString(R.string.buy_offer_wizard_volume))
    ...
    private fun setRxOutputs() {
        viewModel.outPuts.onSuccessRefresh()
                ...
                .subscribe {
                    it?.let { offer ->
                        ...
                        adapter.setLineItems(offer.offerLineItems)
                        adapter.notifyDataSetChanged()
                        offer.offerLineItems?.let { lineItems ->
                            btn_next.isEnabled = lineItems.any { it.offerQty > 0 }
                        }
                    }
                }
        ...
        viewModel.outPuts.onClickNext()
                .bindToLifecycle(this)
                .subscribe {
                    startActivity(Intent(this, BofPriceAct::class.java).putExtra(Intents.OFFER, it))
    ...
    private fun recyclerViewInit() {
        recycler_volume.apply {
            layoutManager = LinearLayoutManager(this@BofVolumeAct)
            adapter = this@BofVolumeAct.adapter
        }
    }

    private fun setListener() {
        // wait click event (toolbar left button)
        appbar_bof_volume.toolbar_left_btn.setSafeOnClickListener{
            onBackPressed()
        }

        appbar_bof_volume.toolbar_done_btn.setSafeOnClickListener {
            showSaveDialog()
        }

        btn_next.setSafeOnClickListener {
            viewModel.inPuts.clickToNext( adapter.getData() )
        }
                ...
                when (chk_batch_input.isChecked) {
                    true -> {
                        ...
                        adapter.getData().offerLineItems?.let { offerLineItems ->
                            offerLineItems[adapterCurrentPosition].offerQty = valueInt
        ...
        when (batchInputMode) {
            ...
            BatchInputMode.ModeBatchInputMax -> {
                ...
                tv_batch_max.setTextColor(ContextCompat
                        .getColor(this@BofVolumeAct, if (isChecked) R.color.greyish_brown else R.color.very_light_pink))
            }
        }
    ...
    private class RecyclerAdapter : RecyclerView.Adapter<RecyclerAdapter.ViewHolder>() {
        ...
        private var lineItems = mutableListOf<OfferLineItem>()
        ...
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
            ...
            return ViewHolder(LayoutInflater.from(parent.context).inflate(R.layout.item_buy_offer_volume, parent, false))
        }

        override fun getItemCount(): Int {
            var size: Int
            lineItems.let { lineItems ->
                size = lineItems.size
            }
            return size
        }
        ...
        fun setLineItems(lineItems: List<OfferLineItem>?) {
            lineItems?.let {
        ...
        fun getLineItems(): List<OfferLineItem> {
        ...
        private fun setInitItemView(itemView: View, position: Int) {
            ...
            lineItems.let { lineItems ->
                with(lineItems[position]) {
                    ...
                    val params = itemView.ll_volume_item_root.layoutParams as FrameLayout.LayoutParams
                    params.setMargins(0, margin, 0, margin)
                    itemView.ll_volume_item_root.layoutParams = params

                    itemView.tv_volume_input_item.visibility = View.GONE
                    itemView.chk_period_item.visibility = View.VISIBLE
                    itemView.et_volume_input_item.visibility = View.VISIBLE

                    itemView.tv_week_item.setTextColor(ContextCompat.getColor(context, R.color.greyish_brown))
                    itemView.et_volume_input_item.inputType = TYPE_NULL
                    ...
                    when (batchInputMode) {
                        ...
                        BatchInputMode.ModeBatchInputYes,
                        BatchInputMode.ModeBatchInputMax -> {
                            itemView.tv_volume_input_item.text = if (offerQty > 0) offerQtyStr else ""
                        }
                    }
                    ...
                    itemView.et_volume_input_item.setBackgroundResource(etOrderVolumeInputItemBackground)
                    itemView.et_volume_input_item.setTextColor(ContextCompat.getColor(context, etOrderVolumeInputItemTextColor))
                }
            }
        }
        ...
        private fun setItemViewBackgroundColor(itemView: View, blink: Boolean) {
            ...
            itemView.ll_volume_item_root.setBackgroundColor(backgroundColor)
        }
        ...
        private fun processVolume(itemView: View, position: Int) {
                                ...
                                if (!isChecked) {
                                    ...
                                    when (batchInputMode) {
                                        ...
                                        BatchInputMode.ModeBatchInputMax -> {
                                            ...
                                            value = itemView.et_volume_input_item.text.toString()
                                            isFocus = true
                                        }
                                    }
            ...
            itemView.et_volume_input_item.setOnTouchListener { v, event ->
                if (batchInputMode == BatchInputMode.ModeBatchInputNo) {
                    when (event.action) {
                        MotionEvent.ACTION_DOWN -> {
                            val value = itemView.et_volume_input_item.text.toString()
        ...
        private fun setVolumeValue(itemView: View, position: Int) {
            lineItems.let { lineItems ->
                with(lineItems[position]) {
                    itemView.et_volume_input_item.setText(if (offerQty > 0) "$offerQty" else "")
                    onUiListener(ListenerKind.LISTENER_VOLUME, position, itemView.et_volume_input_item.text, false)
                }
            }
        }
