...
/** ProgressView is a appbar_progress_sof bar with a flexible text and animations. */
class ProgressView : FrameLayout {
  ...
  private fun setTypeArray(a: TypedArray) {
    ...
    with(this.highlightView) {
      ...
      radius = a.getDimension(R.styleable.ProgressView_progressView_highlightRadius, radius)
  ...
  /** animates [ProgressView]'s appbar_progress_sof bar. */
  fun progressAnimate() {
    this.labelView.x = 0f
  ...
  /** animates [ProgressView]'s appbar_progress_sof bar. */
  fun progressAnimate(destValue: Float) {
    this.labelView.x = 0f
    this.highlightView.updateLayoutParams {
      if (isVertical()) {
        height = 0
    ...
    var value1 = progress / destValue
    progress = destValue

    val animator = ValueAnimator.ofFloat(value1, 1f)
    animator.duration = duration
    animator.addUpdateListener {
      val value = it.animatedValue as Float
      setLabelViewPosition(getLabelPosition() * value)
  ...
  /** sets a appbar_progress_sof change listener. */
  ...
  /** sets a appbar_progress_sof click listener. */
